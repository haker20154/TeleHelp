...
VOLUNTARYCHECK=0
I=1
BOT=tbot  # You can put here other bots. Also you can change it to run more than one bot in the same server.
...
EOF
echo -e "                \e[100m                اسکریپت اتولانچ           \e[00;37;40m"
echo -e "               \e[01;34m                   بازنویسی شده توسط specialteam / @alfred123                 \e[00;37;40m"
echo ""
cat << EOF
...
echo -e "$bld$f4 چک کردن نصب ربات...$rst"
sleep 0.5
ls ../ | grep $BOT > /dev/null
if [ $? != 0 ]; then
  echo -e "$f1 خطا: BOT: $BOT در دایرکتوری یافت نشد$rst"
  sleep 4
  exit 1
fi
echo -e "$f2 $ربات در دایرکتوری یافت شد$rst"
sleep 0.5


echo ""
echo -e "\033[38;5;208m @specialteam_ch -----@alfre123\033[0;00m"
echo -e "\033[38;5;208m @specialteam_ch -----@alfre123\033[0;00m"
echo -e "\033[38;5;208m @specialteam_ch -----@alfre123\033[0;00m"
echo -e "\033[38;5;208m بازنویسی اسکریپت توسط @alfred123\033[0;00m"
echo -e "\033[38;5;208m کانال ما : @specialteam_ch\033[0;00m"
...
echo "$f2 اجرا $CLINUM برای تلگرام سی ال ای$rst"
sleep 0.9

# =====Setup ends===== #

# Opening new tmux in a daemon
echo -e "$bld$f4 استفاده TMUX به عنوانDAEMON...$rst"
...
TMUX= tmux new-session -d -s $BOT "./merbot"
sleep 1.3

CLIPID=`ps -e | grep telegram-cli | head -1 | sed 's/^[[:space:]]*//' | cut -f 1 -d" "`
echo -e "$f2 برنامه تلگرام سی ال ای جدید: $CLIPID$rst"
echo ""
echo ""
...
while true; do
	...
	echo -e "$f2 زمان.چک کردن و اجرا:$f5 $OK $rst"
	echo -e "$f2 تعداد دفعات کرش کرده و ریلانچ شده:$f5 $BAD $rst"
	echo ""
	...
	if [ $NONVOLUNTARY != $NONVOLUNTARYCHECK ] || [ $VOLUNTARY != $VOLUNTARYCHECK ]; then
		echo -e "$f5 ربات اجرا شد!$rst"
		OK=$(( $OK + 1 ))

	else
		echo -e "$f5 ربات اجرا نشد, تلاش برای اجرای دوباره...$rst"
		BAD=$(( $BAD + 1 ))
		sleep 1
		...
		if [ -z "${CLIPID}" ]; then
			echo -e "$f1 خطا: روند تلگرام سی ال ای اجرا نشد$rst"
			echo -e "$f1 تلاش برای  یلانچ نا موفق بود$rst"
			sleep 3
			exit 1
		fi
